...
func (p *Widget) SetSizePolicyHV(horizontal SizePolicyPolicy,vertical SizePolicyPolicy) {
	_drv_ch(CLASSID_WIDGET,_ID_WIDGET_SETSIZEPOLICYHV,unsafe.Pointer(p.info()),unsafe.Pointer(&horizontal),unsafe.Pointer(&vertical),nil,nil,nil,nil,nil,nil,nil)
	return
}
